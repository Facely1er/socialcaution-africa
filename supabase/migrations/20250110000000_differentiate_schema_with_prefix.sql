/*
  # Database Schema Differentiation with Project Prefix
  
  ## Overview
  This migration adds a project-specific prefix to all table names to avoid conflicts with other Supabase projects.
  All tables will be prefixed with 'privacy_' to clearly identify them as belonging to the privacy protection platform.
  
  ## Changes Made
  
  ### Table Renames
  - `users` → `privacy_users`
  - `thirty_day_challenges` → `privacy_thirty_day_challenges`
  - `daily_tasks` → `privacy_daily_tasks`
  - `user_progress` → `privacy_user_progress`
  - `achievements` → `privacy_achievements`
  
  ### Updated References
  - All foreign key constraints updated
  - All indexes renamed with new prefix
  - All RLS policies updated
  - All functions and triggers updated
  - All views updated
  
  ## Security
  - All Row Level Security policies maintained
  - All permissions and grants updated
  - Data integrity preserved through proper foreign key updates
*/

-- Create new tables with privacy_ prefix
...
-- Create indexes for better performance with new names
...
-- Create triggers for updated_at with new table names
...
-- This will only run if the old tables exist
DO $$
BEGIN
  -- Check if old tables exist and migrate data
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users' AND table_schema = 'public') THEN
    -- Migrate users data
    INSERT INTO privacy_users (id, email, first_name, last_name, created_at, updated_at, privacy_profile, preferences)
    SELECT id, email, first_name, last_name, created_at, updated_at, privacy_profile, preferences
    FROM users
    ON CONFLICT (id) DO NOTHING;
    
    -- Migrate thirty_day_challenges data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'thirty_day_challenges' AND table_schema = 'public') THEN
      INSERT INTO privacy_thirty_day_challenges (id, user_id, start_date, current_day, completed_days, streak, total_points, milestones, achievements, created_at, updated_at)
      SELECT id, user_id, start_date, current_day, completed_days, streak, total_points, milestones, achievements, created_at, updated_at
      FROM thirty_day_challenges
      ON CONFLICT (id) DO NOTHING;
    END IF;
    
    -- Migrate daily_tasks data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_tasks' AND table_schema = 'public') THEN
      INSERT INTO privacy_daily_tasks (id, challenge_id, day, title, description, category, difficulty, estimated_time, completed, completed_at, resources, tips, created_at, updated_at)
      SELECT id, challenge_id, day, title, description, category, difficulty, estimated_time, completed, completed_at, resources, tips, created_at, updated_at
      FROM daily_tasks
      ON CONFLICT (id) DO NOTHING;
    END IF;
    
    -- Migrate user_progress data
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_progress' AND table_schema = 'public') THEN
      INSERT INTO privacy_user_progress (id, user_id, total_points, level, current_level_points, next_level_points, streak, last_activity_date, assessment_count, social_shares, badges, created_at, updated_at)
      SELECT id, user_id, total_points, level, current_level_points, next_level_points, streak, last_activity_date, assessment_count, social_shares, badges, created_at, updated_at
      FROM user_progress
      ON CONFLICT (id) DO NOTHING;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'achievements' AND table_schema = 'public') THEN
      INSERT INTO privacy_achievements (id, user_id, achievement_id, title, description, icon, points, category, unlocked, unlocked_at, created_at, updated_at)
      SELECT id, user_id, achievement_id, title, description, icon, points, category, unlocked, unlocked_at, created_at, updated_at
      FROM achievements
      ON CONFLICT (id) DO NOTHING;
    END IF;
  END IF;
